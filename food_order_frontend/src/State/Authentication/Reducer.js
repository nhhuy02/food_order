...
const authReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case ADD_TO_FAVORITES_SUCCESS:
      ...
      const updatedFavorites = Array.isArray(state.favorites) ? state.favorites : [];
      return {
        ...state,
        isLoading: false,
        error: null,
        favorites: isPresentInFavorites(updatedFavorites, action.payload)
          ? updatedFavorites.filter((item) => item.id !== action.payload.id)
